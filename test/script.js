...
function replaceWord(obj) {
    ...
    var allElements = document.querySelectorAll('h1, h2, h3, h4, h5, p, a, caption, span, td, div'); 
 for (var i = 0; i < allElements.length; i++) {
        if (allElements[i].innerText.toLowerCase().includes(obj.keyword.toLowerCase())) {
            if (obj.type == '0') {
                //remove
                allElements[i].innerHTML =
                    allElements[i].innerHTML.replace(obj.keyword, '');
            } else if (obj.type == '1') {
                //replace
                allElements[i].innerHTML =
                    allElements[i].innerHTML.replace(obj.keyword, obj.replace);
            } else if (obj.type == '2') {
                //blur
